        ...
        for path in video_paths:
            ...
            if media_type == "video":
                try:
                    ...
                    if not cap.isOpened():
                        logger.warning(f"Could not open video file via cv2: {source_path}")
                    ...
                    logger.warning(f"OpenCV could not read video duration for {source_path}: {e}")
                    rejected_files.append({"path": path, "reason": "unreadable_video"})
                    continue
        ...
        try:
            ...
            for t in timestamps:
                try:
                    ...
                    if not success:
                        continue
                    # Convert to RGB to keep original scoring logic consistent (frame is HxWx3)
