...
class ClipScoringAgent:
    ...
    def score_video_segment(self, file_path: str, start: float, end: float) -> Dict[str, Any]:
        ...
        if not cap.isOpened():
            logger.warning(f"Could not open video file via cv2: {file_path}")
            return {
                "sharpness": 0.0,
                "motion_score": 0.0,
                "motion_type": "static",
        ...
        try:
            ...
            for t in motion_times:
                try:
                    cap.set(cv2.CAP_PROP_POS_MSEC, t * 1000.0)
                    success1, f1_bgr = cap.read()
                    if not success1:
                        continue
                    
                    cap.set(cv2.CAP_PROP_POS_MSEC, (t + 0.1) * 1000.0)
                    success2, f2_bgr = cap.read()
                    if not success2:
                        continue
